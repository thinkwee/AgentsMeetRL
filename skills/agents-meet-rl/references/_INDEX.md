...
|---|---|---|---|
| Base Frameworks | [base-frameworks.md](base-frameworks.md) | 20 | RL training frameworks for LLM agents (e.g. veRL, OpenRLHF, slime). Pick one as the base of your training stack. |
...
| Search & RAG Agents | [search-rag.md](search-rag.md) | 38 | Agents that interleave reasoning and retrieval. Most use rule-based EM/F1 outcome rewards or info-gain process rewards. |
| Web & GUI Agents | [web-gui.md](web-gui.md) | 29 | Agents that drive browsers, mobile UIs, or desktop OSes. Grounding accuracy and long-horizon planning dominate. |
| Tool-Use Agents | [tool-use.md](tool-use.md) | 20 | Agents that call external APIs/MCP/code interpreters. Reward usually mixes correctness + format + tool-call validity. |
| Code & SWE Agents | [code-swe.md](code-swe.md) | 22 | Software-engineering agents that locate, repair, and test. Most use SWE-bench-style verifiable rewards. |
...
| Embodied Agents | [embodied.md](embodied.md) | 3 | Agents in physical/simulated robotic environments. |
| Domain-Specific Agents | [domain-specific.md](domain-specific.md) | 9 | Specialized verticals: medical, legal, finance, OS tuning. Reward usually mixes domain rules + LLM-judge. |
| Reward Models & Training Methodology | [reward-training.md](reward-training.md) | 6 | Process/outcome reward models, PRM benchmarks, training recipes. |
| Safety / Red-Teaming | [safety.md](safety.md) | 9 | RL for jailbreak attack/defense, tool-call safety, agent guardrails. |
| VLM Agents | [vlm-agent.md](vlm-agent.md) | 19 | Vision-language model agents (visual tool-use, chart QA, autonomous driving, image-based search). |
| Self-Evolution Agents | [self-evolution.md](self-evolution.md) | 10 | Agents that improve themselves through closed-loop feedback (challenger/solver, MCTS-driven self-training). |
| Environments / Benchmarks / Gyms | [environments.md](environments.md) | 47 | Sandbox environments and benchmarks. Pick one matching your task before designing rewards. |
